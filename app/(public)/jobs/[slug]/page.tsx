...
export async function generateMetadata(
  ...
  try {
    ...
    // ✅ Accept only real URLs, ignore base64
    let imageUrl: string | undefined;

    if (
      typeof job.notification_image_base64 === "string" &&
      !job.notification_image_base64.startsWith("data:image")
    ) {
      try {
        const u = new URL(job.notification_image_base64);
        if (u.protocol === "http:" || u.protocol === "https:") {
          imageUrl = job.notification_image_base64;
        }
      } catch {
        // ignore invalid URL
      }
    ...
    const pageUrl = `https://udaanpath.com/jobs/${job.slug}`;

    return {
      title: `${job.title} | UdaanPath`,
    ...
    // ❗ NEVER throw from generateMetadata
    return {
      title: "UdaanPath Jobs",
      description: "Latest government job notifications on UdaanPath.",
      robots: { index: false, follow: false },
    };
  }
}
...
export default async function JobDetailPage({
  ...
  <>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-12">
      ...
      {/* ================= IMAGE ================= */}
      {isValidImageUrl(job.notification_image_base64) && (
        <img
